...
static Order makeMarketOrderBench(OrderId id, Side side, Quantity qty) {
    Order o;
    o.id = id;
    o.side = side;
    o.type = OrderType::Market;
...
TEST(Benchmark, Latency) {
    ...
    auto printStats = [](const char* label, std::vector<long long>& times) {
    ...
    std::cout << "\n=== Order Engine Latency Benchmark ===";
    ...
    // INSERT (no match)
    {
        ...
        for (int i = 0; i < N; i++) {
            ...
            auto order = makeOrder(i + 1, side, price, 100);
    ...
    // MATCH (sweep 10 levels)
    {
        ...
        for (int i = 0; i < N; i++) {
            OrderBook book;
            for (int j = 0; j < 10; j++) {
                book.addOrder(makeOrder(j + 1, Side::Sell, 100 + j, 50));
            }
            auto buy = makeMarketOrderBench(99, Side::Buy, 500);
            times.push_back(timeNs([&]() { book.addOrder(buy); }));
        }
    ...
    // CANCEL
    {
        ...
        OrderBook book;
        for (int i = 0; i < N; i++) {
            book.addOrder(makeOrder(i + 1, Side::Sell, 100 + (i % 50), 100));
        }
        for (int i = 0; i < N; i++) {
            OrderId id = i + 1;
